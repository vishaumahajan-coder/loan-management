...
import { Eye, EyeOff, ArrowLeft, Upload, Gift, User, Phone, Mail, Briefcase, Lock, Shield, ChevronRight } from 'lucide-react';
          ...
          {platformSettings?.borrower_self_registration && (
            <div className="flex bg-slate-100 p-1 rounded-2xl mb-6">
              <button
                ...
                className={`flex-1 py-2.5 text-xs font-black uppercase tracking-widest rounded-xl transition-all ${form.role === 'lender' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
              >
                Lender
              </button>
              <button
                ...
                className={`flex-1 py-2.5 text-xs font-black uppercase tracking-widest rounded-xl transition-all ${form.role === 'borrower' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
              >
                Borrower
              </button>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <Field label="Full Name" icon={User}>
              <input type="text" placeholder="e.g. James Banda" value={form.name} required
                ...
                className="w-full pl-12 pr-4 py-3.5 bg-white border border-gray-100 rounded-2xl text-[14px] font-bold shadow-sm focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all outline-none" />
            </Field>
            ...
            <Field label="Referral Code" icon={Gift}>
              <input type="text" placeholder="DANIEL842 (Optional)" value={form.referralCode}
                ...
                className="w-full pl-12 pr-4 py-3.5 bg-white border border-gray-100 rounded-2xl text-[14px] font-black tracking-widest text-blue-600 placeholder:text-gray-300 placeholder:font-bold placeholder:tracking-normal shadow-sm focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all outline-none" />
            </Field>
            ...
            <Field label="Security Password" icon={Lock}>
              <input type={showPass ? 'text' : 'password'} placeholder="••••••••" value={form.password} required
                ...
                className="w-full pl-12 pr-12 py-3.5 bg-white border border-gray-100 rounded-2xl text-[14px] font-bold shadow-sm focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all outline-none" />
            ...
            <button type="submit"
              className="w-full py-4 rounded-2xl font-black text-white text-[12px] tracking-widest uppercase
                ...
                shadow-[0_10px_30px_rgba(37,99,235,0.25)] transition-all active:scale-[0.98] flex items-center justify-center gap-2 mt-4">
